...
public class UltimateIK : MonoBehaviour {
    ...
    // void Start(){
    //     Targets = new Transform[Objectives.Length];
    //     for(int i=0; i<Targets.Length; i++) {
    //         Targets[i] = GameObject.CreatePrimitive(PrimitiveType.Sphere).transform;
    //         Targets[i].transform.localScale = 0.1f * Vector3.one;
    //         Targets[i].transform.position = Objectives[i].position;
    //         Targets[i].transform.rotation = Objectives[i].rotation;
    //     }
    // }
    void Reset() {
        Root = transform;
    ...
    public static Model BuildModel(Model reference, Transform root, params Transform[] objectives) {
        if(reference == null) {
            return BuildModel(root, objectives);
        }
