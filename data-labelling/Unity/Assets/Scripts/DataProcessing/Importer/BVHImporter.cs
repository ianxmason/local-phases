		...
		using(new EditorGUILayout.VerticalScope ("Box")) {
			...
			using(new EditorGUILayout.VerticalScope ("Box")) {
				...
				using(new EditorGUILayout.VerticalScope ("Box")) {
					...
					if(Utility.GUIButton("Load Single Directory", UltiDraw.DarkGrey, UltiDraw.White)) {
						LoadDirectory();
					}

					if(Utility.GUIButton("Load All Subdirectories", UltiDraw.DarkGrey, UltiDraw.White)) {
						LoadSubDirectories();
					}
	...
	private void LoadSubDirectories() {
		if(Directory.Exists(Source)) {
			...
			for(int j=0; j<subdirectories.Length; j++) {
				DirectoryInfo subinfo = subdirectories[j];
				FileInfo[] items = subinfo.GetFiles("*.bvh");
				for(int i=0; i<items.Length; i++) {
					myfile = new File();
					myfile.Object = items[i];
					myfile.Import = false;
					FileList.Add(myfile);
				}
			}
						...
						for(int k=0; k<data.GetTotalFrames(); k++) {
							...
							for(int i=0; i<data.Source.Bones.Length; i++) {
								...
								for(int j=0; j<channels[i].Length; j++) {
									...
									// Try changing order from rot*=YXZ to rot*=ZXY
									// if(channels[i][j] == 1) {
									// 	position.x = -motions[k][idx]; idx += 1;
									// }
									// if(channels[i][j] == 2) {
									...
									// }
									// if(channels[i][j] == 4) {
									// 	rotation *= Quaternion.AngleAxis(-motions[k][idx], Vector3.left); idx += 1;
									// }
									// if(channels[i][j] == 5) {
									// 	rotation *= Quaternion.AngleAxis(-motions[k][idx], Vector3.up); idx += 1;
									// }
