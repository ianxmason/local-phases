...
public class ContactModule : Module {
	...
	public override Module Initialise(MotionData data) {
		...
		Inspect = true;
		Visualise = true;
		// For faster adding of foot contact labels
		AddContact(22);
		AddContact(27);
		return this;
	}
	...
	public float[] GetFootContacts(Frame frame, bool mirrored){
		float[] conts = new float[2];
		// Right foot contact		
		// conts[0] = Functions[0].HasContact(frame, mirrored);
		conts[0] = System.Convert.ToSingle(Functions[0].HasContact(frame, mirrored));
		// Left foot contact
		// conts[1] = Functions[1].HasContact(frame, mirrored);
		conts[1] = System.Convert.ToSingle(Functions[1].HasContact(frame, mirrored));
		return conts;
	}
	...
	protected override void DerivedDraw(MotionEditor editor) {
		...
		for(int i=0; i<Functions.Length; i++) {
			...
			UltiDraw.DrawSphere(position, Quaternion.identity, Functions[i].DistanceThreshold, UltiDraw.Red.Transparent(0.5f));
		}
		UltiDraw.End();
	}
	...
	public class ContactFunction {
		...
		public void Compute() {
			for(int i=0; i<Module.Data.GetTotalFrames(); i++) {
				RegularContacts[i] = 
									GetPivotVelocity(Module.Data.Frames[i], false).magnitude <= VelocityThreshold && 
									Physics.CheckSphere(GetPivotTransformation(Module.Data.Frames[i], false).GetPosition(), DistanceThreshold, Mask.value);
				InverseContacts[i] = 
									GetPivotVelocity(Module.Data.Frames[i], true).magnitude <= VelocityThreshold && 
									Physics.CheckSphere(GetPivotTransformation(Module.Data.Frames[i], true).GetPosition(), DistanceThreshold, Mask.value);
			}
