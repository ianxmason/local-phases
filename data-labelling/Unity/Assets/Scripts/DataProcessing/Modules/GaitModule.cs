...
public class GaitModule : Module {

	public bool[] Keys = new bool[0];
	public GaitFunction[] Functions = new GaitFunction[0];
	...
	public void AddGait(string name) {
		if(System.Array.Exists(Functions, x => x.Name == name)) {
			Debug.Log("Gait with name " + name + " already exists.");
			return;
		}
		ArrayExtensions.Add(ref Functions, new GaitFunction(this, name));
	}

	public void RemoveGait(string name) {
		int index = System.Array.FindIndex(Functions, x => x.Name == name);
		if(index >= 0) {
			ArrayExtensions.RemoveAt(ref Functions, index);
		} else {
			Debug.Log("Gait with name " + name + " does not exist.");
		}
	}
	...
	public float[] GetGait(Frame frame) {
		float[] gait = new float[Functions.Length];
		for(int i=0; i<gait.Length; i++) {
			gait[i] = Functions[i].GetValue(frame);
		}
		return gait;
	}
	...
	protected override void DerivedInspector(MotionEditor editor) {
		...
		for(int i=0; i<Functions.Length; i++) {
			...
			if(Utility.GUIButton("X", UltiDraw.DarkRed, UltiDraw.White, 20f, 20f)) {
				RemoveGait(Functions[i].Name);
			}
			EditorGUILayout.EndHorizontal();
		}
		if(Utility.GUIButton("Add Gait", UltiDraw.DarkGrey, UltiDraw.White)) {
			AddGait("Gait " + (Functions.Length+1));
	...
	[System.Serializable]
	public class GaitFunction {
		public GaitModule Module;
		public string Name;
		public float[] Values;

		public GaitFunction(GaitModule module, string name) {
			Module = module;
			Name = name;
