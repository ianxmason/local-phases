			...
			using(new EditorGUILayout.VerticalScope ("Box")) {
				...
				if(!Exporting) {
					...
					if(Utility.GUIButton("Export Data", UltiDraw.DarkGrey, UltiDraw.White)) {
						trainX = new Data(WriteData ? CreateFile("InputTrain") : null, WriteNorm ? CreateFile("InputNorm") : null, null);
						trainY = new Data(WriteData ? CreateFile("OutputTrain") : null, WriteNorm ? CreateFile("OutputNorm") : null, null);
						valX = new Data(WriteData ? CreateFile("InputVal") : null, null, WriteLabels ? CreateFile("InputLabels") : null);
						valY = new Data(WriteData ? CreateFile("OutputVal") : null, null, WriteLabels ? CreateFile("OutputLabels") : null);
						testX = new Data(WriteData ? CreateFile("InputTest") : null, null, null);
						testY = new Data(WriteData ? CreateFile("OutputTest") : null, null, null);
						if(ProcTogether){
					...
					if(Utility.GUIButton("Export Data From Subdirectories", UltiDraw.DarkGrey, UltiDraw.White)) {
						...
						testX = new Data(WriteData ? CreateFile("InputTest") : null, null, null);
						testY = new Data(WriteData ? CreateFile("OutputTest") : null, null, null);
						Tr_Va_Te_Frames = CreateFile("Tr_Va_Te_Frames");
			...
			if(Editor.Files[i].Data.GetModule(Module.TYPE.Style) != null) {
				...
				for(int j=0; j<module.Functions.Length; j++) {
					if(!StyleNames.Contains(module.Functions[j].Name)) {
						...
						// if(!(module.Functions[j].Name=="ShieldedLeft")){
						// 	StyleExport.Add(false);
						// }
						// else{
						// 	StyleExport.Add(true);
									...
									for(int j=1; j<states.Count-1; j++) {
										...
										for(int k=0; k<current.Posture.Length; k++) {
											if(k==7 || k==22 || k==27){
												...
												// X.Feed(current.Posture[k].GetRotation().GetRelativeRotationTo(current.Root), Data.ID.Standard, "Bone"+(k+1)+"Rotation");
										...
										for(int k=6; k<7; k++) { // 6 is current
											X.Feed(current.Trajectory.Points[k].Phase, Data.ID.Standard, "Phase");

											for(int localp=0; localp<current.LocalPhases.Length; localp++){
												Vector2 phaseVector = Utility.PhaseVector(current.LocalPhases[localp]);
												X.Feed(phaseVector.x, Data.ID.Standard, "LocalPhasesSine-" + localp);
												X.Feed(phaseVector.y, Data.ID.Standard, "LocalPhasesCosine-" + localp);
											}

											float[] filteredGaits = Filter(current.Trajectory.Points[k].Gaits, gaitmapping, gaits.Length);
											for(int gait=0; gait<gaits.Length; gait++) {
												X.Feed(filteredGaits[gait], Data.ID.Standard, "GaitOneHot -"+gait);
											}

											float[] filteredStyles = Filter(current.Trajectory.Points[k].Styles, stylemapping, styles.Length);
											for(int style=0; style<styles.Length; style++) {
												X.Feed(filteredStyles[style], Data.ID.Standard, "StyleOneHot -"+style);
											}
										}
										...
										//Output. 7-11 are future. 6 is current. 0-5 is past
										for(int k=6; k<12; k++) {
										...
										for(int k=0; k<next.Posture.Length; k++) {
											if(k==7 || k==22 || k==27){
												...
												// Y.Feed(next.Posture[k].GetRotation().GetRelativeRotationTo(current.Root), Data.ID.Standard, "Bone"+(k+1)+"Rotation");
										...
										// }
										for(int k=6; k<7; k++) {
											Y.Feed(Utility.PhaseUpdate(current.Trajectory.Points[k].Phase, next.Trajectory.Points[k].Phase), Data.ID.Standard, "PhaseUpdate");

											// For Y use GetPhase and PhaseVector to get same 2D phase output  as for X but one frame later
											// Also get the PhaseUpdate - not convinced on signedphaseupdate
											for(int localp=0; localp<next.LocalPhases.Length; localp++){
												Vector2 phaseVector = Utility.PhaseVector(next.LocalPhases[localp]);
												Y.Feed(phaseVector.x, Data.ID.Standard, "LocalPhasesSine-" + localp);
												Y.Feed(phaseVector.y, Data.ID.Standard, "LocalPhasesCosine-" + localp);
												// Y.Feed(Utility.PhaseVector(next.LocalPhases[localp]), Data.ID.Standard, "LocalPhases-"+localp);
											}
											for(int localp=0; localp<next.LocalPhases.Length; localp++){
												Vector2 phaseUpdateVector = Utility.PhaseVector(next.LocalPhases[localp]) - Utility.PhaseVector(current.LocalPhases[localp]);
												Y.Feed(phaseUpdateVector.x, Data.ID.Standard, "LocalPhaseUpdatesSine-"+localp);
												Y.Feed(phaseUpdateVector.y, Data.ID.Standard, "LocalPhaseUpdatesCosine-"+localp);
											}

											float[] filteredGaits = Filter(current.Trajectory.Points[k].Gaits, gaitmapping, gaits.Length);
											for(int gait=0; gait<gaits.Length; gait++) {
												Y.Feed(filteredGaits[gait], Data.ID.Standard, "GaitOneHot -"+gait);
											}

											float[] filteredStyles = Filter(current.Trajectory.Points[k].Styles, stylemapping, styles.Length);
											for(int style=0; style<styles.Length; style++) {
												Y.Feed(filteredStyles[style], Data.ID.Standard, "StyleOneHot -"+style);
											}
										}
										Y.Feed(next.FootContacts, Data.ID.Standard, "FootContact");
										Y.Store();
										//
	...
	*/
	...
	public class State {
		...
		public State(MotionEditor editor) {
			...
			LocalPhases = ((LocalPhaseModule)file.Data.GetModule(Module.TYPE.LocalPhase)).GetPhases(frame.Timestamp, editor.Mirror);
		}
	}
