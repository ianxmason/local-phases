...
public static class Utility {
	...
	public static float Normalize(this int value, float valueMin, float valueMax, float resultMin, float resultMax) {
		if(valueMax-valueMin != 0f) {
			return ((float)value-valueMin)/(valueMax-valueMin)*(resultMax-resultMin) + resultMin;
		} else {
