...
public class Trajectory {
	...
	public string[] Gaits = new string[0];

	private static float Width = 0.5f;

	public Trajectory(int size, string[] styles, string[] gaits) {
		Points = new Point[size];
		Styles = styles;
		Gaits = gaits;
		for(int i=0; i<Points.Length; i++) {
			Points[i] = new Point(i, styles.Length, gaits.Length);
			Points[i].SetTransformation(Matrix4x4.identity);
		}
	}

	public Trajectory(int size, string[] styles, string[] gaits, Vector3 seedPosition, Vector3 seedDirection) {
		Points = new Point[size];
		Styles = styles;
		Gaits = gaits;
		for(int i=0; i<Points.Length; i++) {
	...
	public Trajectory(int size, string[] styles, string[] gaits, Vector3[] positions, Vector3[] directions) {
		Points = new Point[size];
		Styles = styles;
		Gaits = gaits;
		for(int i=0; i<Points.Length; i++) {
	...
	public Trajectory(Trajectory trajectory) {
		Points = new Point[trajectory.Points.Length];
		for(int i=0; i<Points.Length; i++) {
			Points[i] = new Point(trajectory.Points[i]);
		}
		Styles = (string[])trajectory.Styles.Clone();
		Gaits = (string[])trajectory.Gaits.Clone();
	}
	...
	public class Point {
		...
		public Point(Point point) {
			...
			Speed = point.Speed;
			LeftSample = point.LeftSample;
			RightSample = point.RightSample;
			Slope = point.Slope;
			Phase = point.Phase;
			// Signals = (float[])point.Signals.Clone();
			Styles = (float[])point.Styles.Clone();
			Gaits = (float[])point.Gaits.Clone();
			GoalPosition = point.GetPosition();
			GoalDirection = point.GetDirection();
		}

		public Point(int index, int styles, int gaits) {
			Index = index;
		...
		public Point(int index, int gaits, int styles, Vector3 seedPosition, Vector3 seedDirection) {
			Index = index;
			Transformation = Matrix4x4.TRS(seedPosition, Quaternion.LookRotation(seedDirection, Vector3.up), Vector3.one);
			Velocity = Vector3.zero;
			LeftSample = Vector3.zero;
			RightSample = Vector3.zero;
			Slope = 0f;
			Styles = new float[styles];
			Gaits = new float[gaits];
			GoalPosition = Transformation.GetPosition();
			GoalDirection = Transformation.GetForward();
		}
	...
	public void DrawSimple(Color color, int step=1) {
		UltiDraw.Begin();

		//Connections
		for(int i=0; i<Points.Length-step; i+=step) {
			UltiDraw.DrawLine(Points[i].GetPosition(), Points[i+step].GetPosition(), 0.01f, color);
		}

		//Positions
		for(int i=0; i<Points.Length; i+=step) {
			UltiDraw.DrawCircle(Points[i].GetPosition(), 0.025f, color);
		}
	...
	public void Draw(int step=1) {
		...
		//Connections
		for(int i=0; i<Points.Length-step; i+=step) {
		// for(int i=0; i<60-step; i+=step) {  // Show only past points - used to create image
