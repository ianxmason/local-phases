...
class PFNNParameter:
    ...
    def load_parameter(self, filename, phase, index):
        nslices = 4
        alpha_const = np.fromfile(filename+'/alpha%0i.bin' % index, dtype=np.float32).reshape(self.weight_shape)
        beta_const = np.fromfile(filename+'/beta%0i.bin' % index, dtype=np.float32).reshape(self.bias_shape)        
    ...
    def load_parameter_from_arr(self, arr, phase, index):
        nslices = 4
        alpha_const = arr[0]
        beta_const = arr[1]      
...
def save_network(alpha, beta, num_points,filename):
    ...
    for i in range(num_points):
        ...
        for j in range(len(alpha)):
            ...
            """extra save the whole matrices for loading later"""
            a.tofile(filename+'/alpha%0i.bin' % j)
            b.tofile(filename+'/beta%0i.bin' % j)
